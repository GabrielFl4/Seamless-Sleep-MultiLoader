...
// Server driver for sleep clock interpolation.
public final class SleepAnimationState {
    ...
    public void tick(ServerLevel world, Holder<WorldClock> clock) {
        if (!this.active) {
            return;
        }
        ...
        if (now <= this.startMillis) {
            world.clockManager().setTotalTicks(clock, this.startTimeOfDay);
            return;
        }

        double x = this.computeProgress(now);
        if (x >= 1.0D) {
            this.active = false;
            world.clockManager().setTotalTicks(clock, this.endTimeOfDay);
            return;
        }

        world.clockManager().setTotalTicks(clock, this.getInterpolatedTime(now));
    }
    ...
    public long getInterpolatedTime(long now) {
        double eased = integralEase(this.computeProgress(now));
        long delta = this.endTimeOfDay - this.startTimeOfDay;
        return this.startTimeOfDay + (long) (delta * eased);
    }

    private double computeProgress(long now) {
